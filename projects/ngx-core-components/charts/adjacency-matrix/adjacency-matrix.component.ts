...
export class AdjacencyMatrixComponent {
  ...
  exportToJson(): void {
    ...
    this.exportSvc.downloadJson(data, 'adjacency-matrix-data.json');
  }
  ...
  exportToCsv(): void {
    ...
    for (let r = 0; r < matrix.length; r++) {
      for (let c = 0; c < matrix[r].length; c++) {
        if (matrix[r][c] > 0) {
          rows.push([labels[r], labels[c], matrix[r][c]]);
        }
      }
    }
    this.exportSvc.downloadCsv(headers, rows, 'adjacency-matrix-data.csv');
  }
