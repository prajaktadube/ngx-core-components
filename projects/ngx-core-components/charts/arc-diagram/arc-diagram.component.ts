import { Component, ChangeDetectionStrategy, input, computed, signal, ElementRef, viewChild, HostListener, inject } from '@angular/core';
...
export class ArcDiagramComponent {
  ...
  exportToJson(): void {
    ...
    this.exportSvc.downloadJson(data, 'arc-diagram-data.json');
  }
  ...
  exportToCsv(): void {
    ...
    const rows: (string | number)[][] = links.map(l => [l.source || '', l.target || '', l.value ?? 0]);
    this.exportSvc.downloadCsv(headers, rows, 'arc-diagram-links.csv');
  }
