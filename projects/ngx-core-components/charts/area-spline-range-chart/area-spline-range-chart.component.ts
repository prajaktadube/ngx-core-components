...
@Component({
  selector: 'ngx-area-spline-range-chart',
  standalone: true,
  imports: [CommonModule],
  changeDetection: ChangeDetectionStrategy.OnPush,
  template: `
    <div class="ngx-area-spline-range-chart" (mousemove)="onMouseMove($event)" (mouseleave)="onMouseLeave()">
      ...
      <div class="chart-svg-wrap">
        <svg #svgEl [attr.width]="'100%'" [attr.height]="height()" class="chart-svg">
          <defs>
            @for (s of series(); track s.name; let i = $index) {
              <linearGradient [attr.id]="'spline-gradient-' + i" x1="0" y1="0" x2="0" y2="1">
          ...
          <g [attr.transform]="'translate(' + PAD_LEFT + ',' + PAD_TOP + ')'">
            ...
            <!-- Range Shaded Areas and Smooth Boundaries -->
            @for (s of series(); track s.name; let si = $index) {
              <!-- Smooth Area Fill -->
              <path
                [attr.d]="areaPath(s)"
                [attr.fill]="'url(#spline-gradient-' + si + ')'"
                stroke="none"
                class="area-path"
              />
              
              <!-- Smooth High Boundary Line -->
              <path
              ...
              <!-- Hover Markers (dots on smooth bounds) -->
              @if (showMarkers() && animateState()) {
                @for (d of s.data; track $index; let ci = $index) {
                  ...
                  <circle
                    [attr.cx]="xPos(ci)"
                    [attr.cy]="yPos(d.high)"
                    [attr.r]="activeCategoryIndex() === ci ? 5.5 : 3.5"
        ...
        <div class="chart-tooltip" [style.left.px]="t.x" [style.top.px]="t.y">
          ...
          @for (row of t.rows; track row.name) {
            ...
            <div class="tt-row detail-row">
              <span class="tt-sub-label">High (Max):</span>
              <span class="tt-val">{{ fmtNum(row.high) }}</span>
            </div>
            <div class="tt-row detail-row">
              <span class="tt-sub-label">Low (Min):</span>
              <span class="tt-val">{{ fmtNum(row.low) }}</span>
            </div>
            <div class="tt-row detail-row range-row">
              <span class="tt-sub-label">Spread Range:</span>
...
export class AreaSplineRangeChartComponent {
  ...
  highLinePath(s: AreaRangeSeries): string {
    const pts: [number, number][] = s.data.map((d, i) => [this.xPos(i), this.yPos(d.high)]);
    return smoothPath(pts);
  }

  lowLinePath(s: AreaRangeSeries): string {
    const pts: [number, number][] = s.data.map((d, i) => [this.xPos(i), this.yPos(d.low)]);
    return smoothPath(pts);
  }
  ...
  areaPath(s: AreaRangeSeries): string {
    ...
    const highPts: [number, number][] = data.map((d, i) => [this.xPos(i), this.yPos(d.high)]);
    const lowPts: [number, number][] = data.map((d, i) => [this.xPos(i), this.yPos(d.low)]);
    
    const highPath = smoothPath(highPts);
    const lowPtsReversed = [...lowPts].reverse();
    const lowPathReversed = smoothPath(lowPtsReversed);
    
    if (!lowPathReversed) return '';
    const lowPathPart = 'L' + lowPathReversed.slice(1);
    return `${highPath} ${lowPathPart} Z`;
  }
  ...
  exportToJson(): void {
    ...
    link.setAttribute('download', 'area-spline-range-chart.json');
  ...
  exportToCsv(): void {
    ...
    const link = document.createElement('a');
    link.href = URL.createObjectURL(blob);
    link.setAttribute('download', 'area-spline-range-chart.csv');
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  }
  ...
  exportToSvg(): void {
    ...
    const link = document.createElement('a');
    link.href = URL.createObjectURL(blob);
    link.setAttribute('download', 'area-spline-range-chart.svg');
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  }
  ...
  exportToPdf(): void {
    ...
    const printTemplate = `
      <html>
      <head>
        <title>Area Spline Range Chart Export</title>
        <style>
      ...
      <body>
        <div class="header">
          <div class="title">Area Spline Range Chart Analysis</div>
