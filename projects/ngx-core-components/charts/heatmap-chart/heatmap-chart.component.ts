...
export class HeatmapChartComponent {
  ...
  exportToJson(): void {
    ...
    this.exportSvc.downloadJson(data, 'heatmap-chart-data.json');
  }
  ...
  exportToCsv(): void {
    ...
    const headers = ['Row_Label', 'Column_Label', 'Value'];
    const rows: (string | number)[][] = [];
    data.forEach((row, rIdx) => {
      const yLabel = yLabels[rIdx] || `Row ${rIdx + 1}`;
      row.forEach((val, cIdx) => {
        const xLabel = xLabels[cIdx] || `Col ${cIdx + 1}`;
        rows.push([yLabel, xLabel, val]);
      });
    });
    this.exportSvc.downloadCsv(headers, rows, 'heatmap-chart-data.csv');
  }

  exportToSvg(): void {
    this.exportSvc.downloadSvg(this.svgEl()?.nativeElement, 'heatmap-chart.svg');
  }

  exportToPdf(): void {
    this.exportSvc.downloadPdf(this.svgEl()?.nativeElement, 'Heatmap Chart', 'heatmap-chart.pdf');
  }
}
