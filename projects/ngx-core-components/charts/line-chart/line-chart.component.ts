...
import { CHART_COLORS, ChartSeries, niceTicks, scale, smoothPath, fmtNum, generateUniqueId } from '../shared/chart-utils';
        ...
        <g [attr.transform]="'translate(' + (PAD_LEFT + panOffset().x) + ',' + (PAD_TOP + panOffset().y) + ') scale(' + zoomScale() + ')'">
          ...
          @for (s of series(); track s.name; let si = $index) {
            @if (showArea()) {
              <path
                [attr.d]="areaPath(s)"
                [attr.fill]="'url(#' + instanceId + '-area-grad-' + si + ')'"
                stroke="none"
                class="area-path"
              />
            }
...
export class LineChartComponent {
  ...
  exportToCsv(): void {
    ...
    const rows = cats.map((cat, ci) => [cat, ...sers.map(s => s.data[ci] ?? '')] as (string | number)[]);
    this.exportSvc.downloadCsv(headers, rows, 'line-chart-data.csv');
  }
  ...
  exportToJson(): void {
    ...
    const data = cats.map((cat, ci) => {
      ...
      sers.forEach(s => { if (s.data[ci] !== undefined && s.data[ci] !== null) entry[s.name] = s.data[ci]; });
      return entry;
    });
    this.exportSvc.downloadJson(data, 'line-chart-data.json');
  }

  exportToSvg(): void {
    this.exportSvc.downloadSvg(this.svgEl()?.nativeElement, 'line-chart.svg');
  }

  exportToPdf(): void {
    this.exportSvc.downloadPdf(this.svgEl()?.nativeElement, 'Line Chart', 'line-chart.pdf');
  }
