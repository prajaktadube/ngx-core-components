...
export class SankeyChartComponent {
  ...
  exportToJson(): void {
    ...
    link.setAttribute('download', 'sankey-chart-data.json');
  ...
  exportToCsv(): void {
    ...
    links.forEach(l => {
      csv += `"${l.source}","${l.target}",${l.value}\n`;
    });
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
    const link = document.createElement('a');
    link.href = URL.createObjectURL(blob);
    link.setAttribute('download', 'sankey-chart-data.csv');
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  }
  ...
  exportToSvg(): void {
    ...
    const link = document.createElement('a');
    link.href = URL.createObjectURL(blob);
    link.setAttribute('download', 'sankey-chart.svg');
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  }
