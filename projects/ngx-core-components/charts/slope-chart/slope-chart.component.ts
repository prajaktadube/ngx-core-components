...
export class SlopeChartComponent {
  ...
  exportToJson(): void {
    ...
    this.exportSvc.downloadJson(data, 'slope-chart.json');
  }

  exportToCsv(): void {
    const data = this.data();
    if (!data.length) return;
    const headers = ['Label', 'Start Value', 'End Value'];
    const rows = data.map(d => [d.label || '', d.startValue, d.endValue]);
    this.exportSvc.downloadCsv(headers, rows, 'slope-chart.csv');
  }
